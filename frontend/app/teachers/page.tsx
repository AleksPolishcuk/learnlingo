...
export default function TeachersPage() {
  ...
  const fetchBatch = useCallback(async (f: TeacherFilters) => {
    const mkParams = (page: number): Record<string, string> => {
    ...
    const [teachersRes, adsRes] = await Promise.allSettled([
      apiDoneRef.current.teachers
        ...
        : api.get("/teachers", {
            params: mkParams(apiPageRef.current.teachers),
          }),
      apiDoneRef.current.ads
        ? Promise.resolve(null)
        : api.get("/teacher-ads", { params: mkParams(apiPageRef.current.ads) }),
    ]);
    ...
    } else if (!apiDoneRef.current.teachers) apiDoneRef.current.teachers = true;
    ...
    } else if (!apiDoneRef.current.ads) apiDoneRef.current.ads = true;

    const merged = [...seeded, ...ads];
    if (f.sortBy === "name_asc")
      merged.sort((a, b) =>
        `${a.name} ${a.surname}`.localeCompare(`${b.name} ${b.surname}`),
      );
    if (f.sortBy === "name_desc")
      merged.sort((a, b) =>
    ...
    if (f.sortBy === "oldest")
      merged.sort(
        (a, b) =>
          new Date((a as any).createdAt ?? Infinity).getTime() -
          new Date((b as any).createdAt ?? Infinity).getTime(),
      );
          ...
          {(hasMore || loading) && (
            <div className={styles.loadMore}>
              <button
                ...
                onClick={() =>
                  showNextPage(filtersRef.current, false, runIdRef.current)
                }
                disabled={loading}
              >
